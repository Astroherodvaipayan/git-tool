...
export function StreamingRepoPanel({ owner, repo }: { owner: string; repo: string }) {
  ...
  // Detect changes (simulated for demo)
  const detectChanges = (newTree: RepoTree, oldTreeJson: string): Set<string> => {
    ...
    if (oldTreeJson !== newTreeJson) {
      ...
      newPaths.slice(0, Math.floor(Math.random() * 3) + 1).forEach(path => changes.add(path));
    }
  ...
  const buildCompleteFileTree = async (owner: string, repo: string, branch: string = "main", path: string = "", depth: number = 0): Promise<RepoTree> => {
    if (depth > 3) {
      return { type: "folder", path, contents: {} };
    }

    const files = await getRepoContents(owner, repo, path);
    const tree: RepoTree = { type: "folder", path, contents: {} };
  ...
  const prepareVisualizationData = (tree: RepoTree) => {
    const rootNode = { name: repo, children: [], value: 0, path: "", type: "folder" };
  ...
  const renderVisualization = (data: any) => {
    ...
    // Animate circles appearing with elastic effect
    node.append("circle")
      .attr("r", 0)
      .style("fill", (d: any) => {
        if (!d.children) {
      ...
      .on("end", function(d: any) {
        // Highlight changed files with red pulse
  ...
  const getLanguageFromPath = (path: string) => {
    ...
    const languageMap: { [key: string]: string } = {
      ...
      'php': 'php', 'cs': 'csharp', 'html': 'html', 'css': 'css', 'scss': 'scss',
      'json': 'json', 'md': 'markdown', 'yml': 'yaml', 'yaml': 'yaml', 'xml': 'xml',
      'sh': 'shell', 'bash': 'shell', 'c': 'c', 'cpp': 'cpp', 'h': 'c', 'hpp': 'cpp', 'sql': 'sql'
    };
    ...
    return (
      <div key={fullPath} className="select-none">
        <div
          ...
          } ${hasChanges ? "bg-blue-50 border-l-4 border-blue-500 animate-pulse" : ""}`}
          onClick={() => {
            if (isFolder) {
        ...
        >
          <div className="flex items-center min-w-0 flex-1">
            {isFolder ? (
              expanded ? (
                <FolderOpen className={`w-4 h-4 mr-2 flex-shrink-0 ${hasChanges ? 'text-blue-600' : 'text-blue-500'}`} />
              ) : (
                <FolderIcon className={`w-4 h-4 mr-2 flex-shrink-0 ${hasChanges ? 'text-blue-700' : 'text-blue-600'}`} />
              )
            ) : (
              <FileIcon className={`w-4 h-4 mr-2 flex-shrink-0 ${hasChanges ? 'text-blue-600' : 'text-gray-500'}`} />
            )}
            <span className="truncate">{key}</span>
            {hasChanges && (
              <Badge variant="secondary" className="ml-2 text-xs animate-bounce bg-blue-100 text-blue-800">
                ✨ Updated
              </Badge>
            )}
            {!isFolder && node.size && (
  ...
  if (error) {
    return (
      <Card className="h-full">
        <CardHeader>
          <CardTitle>🌊 Real-time Repository Stream</CardTitle>
          <CardDescription>Streaming visualization error</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <Button onClick={() => fetchRepoStructure()} className="mt-4">
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Streaming visualization of <span className="font-semibold text-primary">{repo}</span> repository structure
            </CardDescription>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-6 text-sm text-muted-foreground bg-muted/30 rounded-lg p-3 mt-2">
          ...
          <span>Recent changes: {streamingMetrics.filesChanged}</span>
        </div>
      </CardHeader>
      <CardContent>
        {fileTree ? (
                ...
                <div className="w-full lg:w-1/2">
                  <Card className="h-full">
                    <CardHeader className="pb-3">
                      ...
                      <CardDescription>Real-time repository structure updates every 10 seconds</CardDescription>
                    </CardHeader>
                ...
                <div className="w-full lg:w-1/2">
                  {selectedContent ? (
                    <Card className="h-full">
                      ...
                      <CardContent className="p-0">
                        <ScrollArea className="h-[580px] border-t">
                          <pre className="p-4 text-xs font-mono bg-gradient-to-br from-muted/20 to-muted/40 whitespace-pre-wrap overflow-x-auto">
                    ...
                    <Card className="h-full flex items-center justify-center bg-gradient-to-br from-muted/20 to-muted/40">
                      <CardContent className="text-center">
                        <FileIcon className="w-16 h-16 text-muted-foreground mx-auto mb-4 opacity-50" />
                        <p className="text-muted-foreground text-lg">Select a file to view its contents</p>
                        <p className="text-muted-foreground text-sm mt-2">Click on any file in the tree or visualization</p>
                      </CardContent>
                    </Card>
                  )}
                </div>
            ...
            <TabsContent value="visualization" className="mt-0">
              <Card className="h-[700px]">
                <CardHeader className="pb-3">
                  ...
                  <CardDescription>
                    Real-time animated visualization with elastic animations. File sizes determine circle size. Changed files pulse red.
                  </CardDescription>
                </CardHeader>
                <CardContent className="p-0">
                  <div className="border-t bg-gradient-to-br from-blue-50 to-purple-50 dark:from-blue-950/20 dark:to-purple-950/20 h-[580px] relative overflow-hidden">
                    <svg ref={svgRef} width="100%" height="100%" className="absolute inset-0" />
                  </div>
                  {selectedContent && (
                    <div className="border-t p-4 bg-gradient-to-r from-muted/30 to-muted/50">
                      <div className="flex items-start gap-3">
                        <FileIcon className="w-5 h-5 text-primary flex-shrink-0 mt-0.5" />
                        <div className="min-w-0 flex-1">
                          <h4 className="font-medium truncate text-primary">{selectedContent.path}</h4>
                          ...
                          <ScrollArea className="mt-3 h-32">
                            <pre className="text-xs font-mono bg-background/90 p-3 rounded-lg border whitespace-pre-wrap shadow-sm">
                              {selectedContent.content}
                            </pre>
                          </ScrollArea>
                        </div>
